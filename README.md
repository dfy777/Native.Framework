...
從3版本至4版本的變化相當的大，如果你是正在使用 <kbd>SDK 3</kbd> 甚至 <kbd>SDK 2</kbd> 的用戶可能會比較迷惘和困惑，希望以下的變化概要能對你有幫助。

<h3 id="changelog">變化概要</h3>

- <kbd>SDK 1</kbd><kbd>SDK 2</kbd><kbd>SDK 3</kbd> 與舊版本不相容，若在酷Q下運行甚至放置(即不啟用)一些舊版本與<kbd>SDK 4</kbd>，酷Q會沒有回應並在``20``秒後跳出錯誤提示。**必須**把所有應用之SDK版本升級才可解決。:neutral_face:

- <kbd>SDK 2</kbd><kbd>SDK 3</kbd> ``Interface`` 都放到 ``Native.Csharp.Sdk``項目裏了，而[Native.Csharp.Sdk](https://www.nuget.org/packages/Native.Csharp.Sdk/) 可以在``Nuget``安裝了！:cupid:

- <kbd>SDK 1</kbd><kbd>SDK 2</kbd> ``Native.Csharp.App.CQMain``是酷Q應用主入口類，所有事件**必須**繼承事件介面``Native.Csharp.Sdk/Cqp/Interface/I*``並以應用json(``Native.Csharp/Native.Csharp.json``)文件中的相應事件屬性``name``命名註冊。註冊命名配對主要作用是針對多個相同類別事件的區分,比如事件具不一優先性或是正則特性處理各自反響之用。:star:
...
- <kbd>SDK 2</kbd> 好友群添加請求的識別flag 從 ``e.Tag`` 更名至 ``e.ResponseFlag`` 了:no_mouth:
